...
def normal_to_shifted_lognormal(k, f, s, t, v_n):
    ...
    if abs(k-f) < eps:
        v_sln_0 = v_n / f+s    #v_sln_0 = shifted lognormal vol
    ...
    def premium_square_error(v_sln):
        premium = n * shifted_lognormal_call(k, f, s, t, v_n, 0.)
    ...
    res = minimize(
        ...
        x0 = v_sln_0,
        jac = None,
        options = {'gtol': 1e-8, #gradient norm < gtol before successful termination
                   'eps': 1e-9, #absolute step size 
                   'maxiter': 10, #max number of iter to perform
